...
def run_local():
    config = Config()
    catchup(config.bucket, config.key_prefix, config.connection_string)


def catchup(bucket_name, raw_path, connection_string):
    datebucket = '2017/05/02'
    for i in range(20, 21):
        for j in range(0, 23):
            hourbucket = "%02d%02d" % (i, j)
            path = raw_path + "/" + datebucket + "/" + hourbucket
            print "parsing %s" %path
            run(bucket_name, path, connection_string)
